...
void swap_a(t_stack **a)
{
	...
	second_node = (*a)->next; // guarda ponteiro segundo node (N2) numa variavel auxiliar; 
...
void swap_b(t_stack **b)
{
	...
	second_node = (*b)->next; // guarda ponteiro segundo node (N2) numa variavel auxiliar; 
	temp->next = second_node->next; // ponteiro next de N1 que apontava para N2 passa a apontar para N3; 
	second_node->next = temp; // ponteiro next de N1 que apontava para N3 passa a apontar para N1;
...
void	rev_rotate_a(t_stack **a)
{
	...
	{
		prev = last; // obter penultimo node
		last = last->next; // ultimo node
	}
	prev->next = NULL; // atribuir ao ultimo node o valor NULL 
	last->next = *a; // primeiro node passa a ser o segundo
	*a = last; // primeiro node passa a ser o ultimo
    ft_printf("rra\n");
}
